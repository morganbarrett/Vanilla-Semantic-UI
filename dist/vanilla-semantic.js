...
;(function(window, document, undefined){
	...
	vs.dimmer = function(selector, parameters){
		var modules = vs.parse(selector),
			time = new Date().getTime(),
			performance = [];

		modules.forEach(function(element){
			var settings = vs.extendSettings("dimmer", parameters),
				selector = settings.selector,
				namespace = settings.namespace,
				className = settings.className,
				error = settings.error,
...
/*!
 * # Vanilla Semantic UI 0.0.1 - Dropdown
 * http://github.com/semantic-org/semantic-ui/
 *
 *
 * Released under the MIT license
...
;(function(vs, window, document, undefined){
	...
	vs.nag = function(selector, parameters){
		var modules = vs.parse(selector),
			time = new Date().getTime(),
			performance = [];

		modules.forEach(function(element){
			var settings = vs.extendSettings("nag", parameters),
				className = settings.className,
				selector = settings.selector,
				error = settings.error,
				namespace = settings.namespace,
